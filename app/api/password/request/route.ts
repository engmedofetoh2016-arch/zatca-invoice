...
export async function POST(req: Request) {
  ...
  if (user) {
    ...
    if (hasSmtp()) {
      ...
      const text = `استخدم هذا الرابط لإعادة تعيين كلمة المرور: ${resetUrl}`
      const html = `<p>استخدم هذا الرابط لإعادة تعيين كلمة المرور:</p><p><a href="${resetUrl}">${resetUrl}</a></p>`
